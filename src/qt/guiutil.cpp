...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("flexcoin"))
        return false;
...
{
    // Convert flexcoin:// to flexcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("flexcoin://"))
    {
        uri.replace(0, 12, "flexcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "FlexCoin.lnk";
}
...
{
    return GetAutostartDir() / "FlexCoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("FlexCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  FlexCoin-qt [" + tr("command-line options") + "]                     " + "\n";
